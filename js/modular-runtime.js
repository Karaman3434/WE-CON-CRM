...
(function(global){
  ...
  /* Müşteri kartındaki gereksiz tam liste localStorage yazımını kaldırır. */
  function fixCustomerCardFreeze(){
    if(typeof global.musteriKartAc!=='function'||global.__WEICON_CUSTOMER_CARD_FREEZE_FIX__)return;
    var source=global.musteriKartAc.toString();
    var re=/\n[ \t]*musteriListesiniKaydet\(\);/;
    if(!re.test(source))return;
    try{global.musteriKartAc=(new Function('return ('+source.replace(re,'')+')'))();global.__WEICON_CUSTOMER_CARD_FREEZE_FIX__=true;}catch(e){console.error(e);}
  }

  /* İşlem Geçmişi: eski ağır renderer tamamen bypass edilir.
     Sadece RAM'deki arsivData okunur; localStorage JSON.parse yoktur.
     Bu fonksiyon bilinçli olarak hafiftir ve müşteri hafızasına dokunmaz. */
  function renderHistorySafe(){
    var el=document.getElementById('gecmisIslemlerListesi');
    if(!el)return;
    var idx=global.musteriKartIdx;
    var m=Array.isArray(global.musteriListesi)?global.musteriListesi[idx]:null;
    if(!m){el.innerHTML='<div style="padding:20px;color:#888;font-size:24px">Müşteri bulunamadı.</div>';return;}
    var a=archive(),types=['numune','teklif','proforma','siparis'];
    ...
    for(var t=0;t<types.length;t++){
      var list=Array.isArray(a[types[t]])?a[types[t]]:[];
      for(var i=0;i<list.length;i++){
        var k=list[i];
        var same=(m.id&&k.musteriId)?String(k.musteriId)===String(m.id):String(k.musteri||'').trim().toLocaleLowerCase('tr-TR')===String(m.ad||'').trim().toLocaleLowerCase('tr-TR');
        if(same)rows.push({tip:types[t],idx:i,k:k,ts:Number(k.ts)||0});
      }
    }
    ...
    for(var r=0;r<max;r++){
      var x=rows[r],k=x.k,u=Array.isArray(k.urunler)?k.urunler:[],total=0;
      for(var q=0;q<u.length;q++)total+=Number(u[q].toplamEuro)||0;
      var date=k.tarih||'';
      html+='<div style="border:2px solid '+colors[x.tip]+';border-radius:10px;padding:11px 12px;margin:0 0 9px;background:#fff">'+
        '<div style="display:flex;justify-content:space-between;gap:8px;align-items:center">'+
        '<span style="background:'+colors[x.tip]+';color:#fff;border-radius:14px;padding:4px 9px;font-size:12px;font-weight:900">'+labels[x.tip]+'</span>'+
        '<span style="font-size:13px;color:#666;font-weight:800">'+esc(date)+'</span></div>'+
        '<div style="margin-top:7px;font-size:15px;font-weight:900;color:#003a70">'+esc(k.kod||k.kodNo||'-')+'</div>'+
        '<div style="margin-top:5px;font-size:14px;color:#333">'+(u.length?esc(u.map(function(z){return (z.name||z.ad||'Ürün')+(z.adet!=null?' × '+z.adet:'');}).join(' · ')):'Ürün bilgisi yok')+'</div>'+
        '<div style="margin-top:6px;font-size:15px;font-weight:900;color:#222">Toplam: '+money(total)+'</div></div>';
    }
  ...
  function openHistorySafe(){
    ...
    var m=global.musteriListesi&&global.musteriListesi[global.musteriKartIdx];if(!m)return;
    ...
    var el=document.getElementById('gecmisIslemlerListesi');if(el)el.innerHTML='<div style="padding:18px;color:#777;font-size:20px">İşlem geçmişi yükleniyor…</div>';
    global.requestAnimationFrame(function(){try{renderHistorySafe();if(typeof global.dispatchEvent==='function')global.dispatchEvent(new Event('weicon:customer-history-open'));}catch(e){console.error('[WE-CON-CRM] History:',e);if(el)el.innerHTML='<div style="padding:20px;color:#c0392b;font-size:20px">İşlem geçmişi yüklenemedi.</div>';}});
  }

  async function boot(){
    for(const src of MODULES){try{await load(src);state.loaded.push(src);}catch(e){state.failed.push({src:src,message:e.message});}}
    fixCustomerCardFreeze();
    global.musteriGecmisRenderEt=renderHistorySafe;
    global.musteriGecmisIslemleriAc=openHistorySafe;
    global.__WEICON_HISTORY_SAFE_MODE__=true;
    state.status=state.failed.length?'degraded':'ready';
    state.finishedAt=Date.now();
    var registry=global.WEICONCRM&&global.WEICONCRM.modules;
    ...
    console.info('[WE-CON-CRM] Modular runtime:',state.status,state.loaded.length+'/'+MODULES.length,'history-safe-mode');
  }
