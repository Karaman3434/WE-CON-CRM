...
function aySiparisleriAc(ayIndex){
  ...
  for(var i=0;i<ay.kayitlar.length;i++){
    ...
    // birebir aynı. 2. satırın sonunda TOPLAM (kart rengi) / PRİM (yeşil) kısa
    // biçimde, "·" ile ayrılmış olarak yer alıyor.
    satirlar += "<div onclick=\"document.getElementById('aySiparisleriModal').style.display='none';arsivDetayAc('siparis',"+it.idx+")\" style='cursor:pointer;border-radius:12px;background:linear-gradient(135deg,"+zeminGrad+");border:2.5px solid "+kartRenk+";padding:10px 16px;margin-bottom:10px;overflow:hidden;"+(sorunluMu?"opacity:.9;":"")+"'>"
      +"<div style='display:flex;align-items:baseline;gap:9px;white-space:nowrap;overflow:hidden;margin-bottom:4px;'>"
        ...
        +"<span style='font-size:19px;font-weight:800;color:#556170;flex-shrink:0;'>"+tarihKisaltTekSatir(it.kayit.tarih)+"</span>"
      ...
      +"<div style='display:flex;align-items:baseline;gap:8px;white-space:nowrap;overflow:hidden;'>"
        +"<span style='font-size:26px;font-weight:900;color:"+kartRenk+";overflow:hidden;text-overflow:ellipsis;'>"+safeText(it.kayit.musteri||"-")+"</span>"
        +"<span style='font-size:20px;font-weight:900;flex-shrink:0;margin-left:auto;padding-left:8px;'><span style='color:"+kartRenk+";'>"+fmt(it.toplam)+" €</span><span style='opacity:.4;'> · </span><span style='color:"+(sorunluMu?kartRenk:"#16a085")+";'>"+fmt(it.prim)+" €</span></span>"
      +"</div>"
      +"</div>";
  }
  var html = "<div style='font-size:28px;font-weight:900;color:#003a70;margin-bottom:14px;text-align:center;border-bottom:3px solid #f2994a;padding-bottom:10px;'>📅 "+ay.ayAd+" "+ay.yil+" Siparişleri</div>"
    +"<div style='background:linear-gradient(135deg,#0a1628,#132840);border-radius:14px;padding:18px 20px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;'>"
      +"<span style='font-size:19px;font-weight:800;color:#7fd6ff;'>🧮 AY TOPLAMI ("+ay.kayitlar.length+" sipariş)</span>"
      +"<span style='font-size:26px;font-weight:900;color:#00d9ff;'>"+fmt(ayToplam)+" €</span>"
    +"</div>"
    +satirlar;
