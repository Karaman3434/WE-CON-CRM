...
- `assets/styles.css` is not yet present on `main`.
- The planned CSS extraction has NOT been applied to `main`.
...
- CSS extraction: planned as the first low-risk refactor step, but NOT yet applied to `main`.
- `PROJECT_CONTEXT.md`: created on the `project-context` branch as the first project-memory artifact.

## 6. Working principle for future AI-assisted development
