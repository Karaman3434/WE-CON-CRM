...
const memoryService = fs.readFileSync(path.join(root, 'js/customers/customer-memory-read-service.js'), 'utf8');
const registry = fs.readFileSync(path.join(root, 'js/core/module-registry.js'), 'utf8');
...
assert(runtime.includes('js/customers/customer-memory-read-service.js'), 'runtime includes customer memory read service');
...
assert(memoryService.includes('getCustomerSnapshot'), 'customer memory read service exposes snapshot');
assert(registry.includes('global.WEICONCRM.modules'), 'runtime registry namespace exists');
assert(runtime.includes('global.WEICONCRM && global.WEICONCRM.modules'), 'runtime uses the actual registry namespace');
